...
#ifdef CONFIG_KSU_SUSFS
/*
 * NOTE: The SUS_SU feature (and its state-tracking helpers
 * susfs_set_current_proc_no_su() / susfs_set_current_proc_umounted_for_zygote_next()
 * / susfs_extra_works) was deprecated and removed starting SUSFS v2.0.0
 * and is no longer provided by fs/susfs.c. Only susfs_set_current_proc_umounted()
 * (the plain SUS_MOUNT umount marker) still exists, so that's all we call here.
 */
